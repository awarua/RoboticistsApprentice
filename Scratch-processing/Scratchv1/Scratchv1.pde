...
int floorHeight = 580;
float turnfactor = 0;
float lastx1, lasty1, lastx2, lasty2, lastx3, lasty3, lastx4, lastscratchC;
int[] tile1 = {80, 202, 0};
int[] tile2 = {271, 291, 0};
int[] tile3 = {444, 191, 0};
int[] tile4 = {249, 110, 0};
...
void draw() {
  ...
  botY = fullblob[1];
  botR = heading;
  float newscratchX = (scratchX+(480/2))*570/480;
  float newscratchY = ((scratchY-(360/2))*-1)*570/360;
  ...
  if ((scratchC != 6)&&(scratchstart == 1)) {
    fill(c);
    ellipse(newscratchX, newscratchY, 20, 20);
  }
  if (botstart == 1) {
    ...
    if (((heading-newscratchR)<-5)||((heading-newscratchR)>5)) {
      turn = true;
      botT = 0;
      println("turning");
    }
    ...
    if ((((fullblob[0]-newscratchX)<-12)||((fullblob[0]-newscratchX)>12)||((fullblob[1]-newscratchY)<-12)||((fullblob[1]-newscratchY)>12))&&(turn == false)) {
    ...
    println("scratchY "+newscratchY);
    println("fullblob "+fullblob[1]);
    println("scratchx "+newscratchX);
    println("fullblob "+fullblob[0]);
